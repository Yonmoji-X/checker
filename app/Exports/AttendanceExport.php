...
class AttendanceExport implements FromCollection, WithHeadings, WithColumnWidths
{
    ...
    //             $data['補足'] = $contentItems;
    //         }

    //         return $data;
    //     });

    //         // ⭐ 最終行に Excel の SUM 関数を追加
    //     $lastRowIndex = $records->count() + 1; // headings が 1行目にある場合
    //     $records->push([
    //         '氏名' => '合計',
    //         '日付' => '',
    //         '出勤時刻' => '',
    ...
    //         '休憩時間' => '',
    //         '実質労働' => '合計',
    //         // G列（実質労働（時間））の2行目～最終行までを SUM
    //         '実質労働（時間）' => '=SUM(INDIRECT(ADDRESS(2,COLUMN())&":"&ADDRESS(ROW()-1,COLUMN())))',
    //         '備考' => '',
    //         '補足' => '',
    //     ]);
    // }
    ...
    public function collection()
    {
        ...
        $records = $query->get()->map(function($attendance) {
            ...
            $workDuration = $clockIn->diffInMinutes($clockOut);
            $breakDuration = $attendance->breakSessions->sum('break_duration');
            $actualWorkDuration = max($workDuration - $breakDuration, 0);
        ...
        SUBSTITUTE(INDIRECT("E"&ROW()), "分", "")/60, 2)';
            ...
            $contentItems = $attendance->records
                ->filter(fn($r) => $r->template && $r->template->export == 1)
                ->pluck('content_item')
                ->implode("\n");
